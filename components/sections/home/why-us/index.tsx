import { resolveWhyUsIcon } from "@/lib/why-us-icons";
import type { HomeWhyUsPillar } from "@/lib/sanity.home";
...
export type WhyUsSectionProps = {
  ...
  headline?: string;
  intro?: string;
  pillars?: HomeWhyUsPillar[];
};

/** Homepage “Why us” / differentiation strip (three pillars). */
export function WhyUsSection({
  eyebrow = fb.whyUsEyebrow!,
  headline = fb.whyUsHeadline!,
  intro = fb.whyUsIntro!,
  pillars = fb.whyUsPillars!,
}: WhyUsSectionProps) {
  const cards = pillars.slice(0, 3);
    ...
    >
      ...
      <div className="site-container relative py-16 sm:py-20 lg:py-24">
        ...
        <div className="mt-12 grid gap-5 sm:mt-14 sm:gap-6 lg:mt-16 lg:grid-cols-3">
          {cards.map(({ title, body, iconKey }, i) => {
            const Icon = resolveWhyUsIcon(iconKey);
            return (
              <article
                key={`${title}-${i}`}
                className={cn(
